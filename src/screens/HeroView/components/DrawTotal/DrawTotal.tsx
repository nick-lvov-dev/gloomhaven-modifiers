  ...
  return total ? (
    <View style={[styles.container, style]}>
      {typeof total.attack === 'number' ? (
        <>
          <RoundIcon image={valueIcons.attack} style={styles.icon} />
          <Text style={[styles.text, { color }]}>{`${total.attack >= 0 ? '+' : ''}${total.attack.toString()}`}</Text>
        </>
      ) : null}
      {total.heal ? (
        <>
          <RoundIcon image={valueIcons.heal} style={styles.icon} />
      ...
      {total.effects
        ? total.effects.map(x =>
            roundEffects.includes(x) ? (
              <RoundIcon image={effectIcons[x]} style={styles.icon} key={`total_effect_icon_${id.current}_${x}`} />
            ) : (
              <SquareIcon image={effectIcons[x]} style={styles.icon} />
            )
          )
        : null}
    </View>
